...
//typedef ap_axis <32, 0, 0, 0> MAC_AXIS_DATA;
						...
						uint L2_TILENUM_K, uint L2_TILENUM_C, uint L2_TILENUM_R, uint L2_TILENUM_S)
{
		...
		dma_control[mm2s_source_addr_reg>>2] = weight_addr +
									((kmo*(L2_TILENUM_C)*(L2_TILENUM_R)*(L2_TILENUM_S)+cmo*(L2_TILENUM_R)*(L2_TILENUM_S)+rmo*(L2_TILENUM_S)+smo)
										*(K_L2/VEC_SIZE)*C_L2*R_L2*S_L2 )*VEC_SIZE ;
		dma_control[mm2s_length_reg>>2] = (K_L2)*C_L2*R_L2*S_L2; //1bytes * len(weight)
						...
						uint C_L2, uint H_L2, uint W_L2, uint H_in_L2, uint W_in_L2, uint WH_in)
{
		...
		for (unsigned int co = 0; co < C_L2/VEC_SIZE; co++) {
			...
			for(unsigned int h = 0; h < H_in_L2; h++) {
				...
				dma_control[mm2s_source_addr_reg>>2] = data_addr + ((cmo*(C_L2/VEC_SIZE)+co)*WH_in*WH_in + (hmo*H_L2+h)*WH_in + (wmo*W_L2))*VEC_SIZE;
				dma_control[mm2s_length_reg>>2] = W_in_L2*VEC_SIZE; //4bytes * len(conv_out)
						...
						uint K_L2, uint H_L2, uint W_L2, uint WH)
{
		...
		for (unsigned int ko = 0; ko < (K_L2 / VEC_SIZE); ko++) {
			...
			for (unsigned int h = 0; h < H_L2; h++) {
				...
				dma_control[mm2s_source_addr_reg>>2] = output_addr + (((kmo*(K_L2/VEC_SIZE)+ko)*WH*WH + (hmo*H_L2+h)*WH + (wmo*W_L2))*VEC_SIZE)*4;
...
void bias_dram_read(uint bias_addr, volatile uint *dma_control, hls::stream<AXIS_DATA> &in_data, uint K)
{
	//BIAS_DRAM_READ: for (unsigned int ko = 0; ko < K/VEC_SIZE; ko++) {
	BIAS_DRAM_READ: for (unsigned int ko = 0; ko < K/16; ko++) {
		#pragma HLS loop_tripcount min=1 max=1
#pragma HLS pipeline off
		//set dma addr&length : bias
	//uint ko = 0;
		dma_control[mm2s_source_addr_reg>>2] = bias_addr + ko*VEC_SIZE;
		dma_control[s2mm_length_reg>>2] = 0; //4bytes * len(conv_out)
		dma_control[mm2s_length_reg>>2] = VEC_SIZE; //K; //1bytes * len(bias)
		AXIS_DATA temp;
		in_data.read(temp);
...
//#pragma HLS pipeline off
			...
			bias_l2[ki][ko] = (temp.data>>(ki*8)) & 0x000000FF;
		}
	}
}

void bias_dram_write(uint output_addr, volatile uint *dma_control, hls::stream<AXIS_DATA> &out_data,
		uint ko_, uint wh, uint WH, uint K)
{
	BIAS_DRAM_WRITE_K: for (unsigned int ko = 0; ko < K/VEC_SIZE; ko++) {
	#pragma HLS loop_tripcount min=1 max=1
	#pragma HLS pipeline off
//#pragma HLS pipeline off
	//set dma addr&length : conv out
	dma_control[s2mm_dest_addr_reg>>2] = output_addr + 0;//((ko*(WH*WH)+wh)*VEC_SIZE)*4;
	//dma_control[mm2s_source_addr_reg>>2] = output_addr + ((ko*(WH*WH)+wh)*VEC_SIZE)*4;
	//dma_control[s2mm_length_reg>>2] = (K*WH*WH)*4; //4bytes * len(conv_out)
	dma_control[s2mm_length_reg>>2] = VEC_SIZE<<2; //4bytes * len(conv_out)
	//dma_control[mm2s_length_reg>>2] = 0; //4bytes * len(conv_out)
	int datas[VEC_SIZE*4];
#pragma HLS ARRAY_PARTITION variable=datas dim=0 complete
	for (unsigned int ki = 0; ki < VEC_SIZE; ki++) {
	#pragma HLS unroll
	//#pragma HLS pipeline off
...
//			unsigned int global_k = (ko)*VEC_SIZE + ki;
		...
		int write_data = bias_l2[ki][ko];
		datas[ki] = write_data;
	}

	for (unsigned int ki = 0; ki < VEC_SIZE; ki++) {
		int write_data = datas[ki];
		AXIS_DATA temp;
		//temp.last = 0; temp.data = (write_data & 0x000000FF); out_data.write(temp); //little endian
		//temp.last = 0; temp.data = (write_data & 0x0000FF00)>>8; out_data.write(temp);
		//temp.last = 0; temp.data = (write_data & 0x00FF0000)>>16; out_data.write(temp);
		temp.keep = 0xf;
		temp.strb = 0xf;
		temp.last = (ki%VEC_SIZE==VEC_SIZE-1);//(ki%4==3/*max burst*/ || ki==VEC_SIZE-1/*global_khw == K*WH*WH-1*/);
						temp.data = write_data;/*(write_data & 0xFF000000)>>24;*/ out_data.write(temp);
	}
...
void write(
		volatile unsigned int* dma_control,uint signal)
{
...
void Conv_sysarr(
	...
	*/
	...
	//reset channel
	volatile uint signal = 0x04;
	write(dma_control, signal);
	//halt channel
	volatile uint signal2 = 0x00;
	write(dma_control, signal2);

	/*volatile uint signal2 = 0x00;
	dma_control[s2mm_ctrl_reg>>2] = signal2;
	dma_control[mm2s_ctrl_reg>>2] = signal2;
	status = dma_control[mm2s_status_reg>>2];
	while(!(status!=0)) {
		status = dma_control[mm2s_status_reg>>2];
	}
	...
	volatile uint signal3 = 0xf001;
	dma_control[s2mm_ctrl_reg>>2] = signal3; //s2mm channel running
	dma_control[mm2s_ctrl_reg>>2] = signal3; //mm2s channel running

	/*
	//set dma addr
	dma_control[mm2s_source_addr_reg>>2] = input_addr + ((transfn + intervn)*r)*4;
	//dma_control[s2mm_dest_addr_reg>>2] = output_addr + ((transfn + intervn)*r)*4;
	//set dma length
	dma_control[mm2s_length_reg>>2] = transfn<<2; //4bytes
	//dma_control[s2mm_length_reg>>2] = transfn<<2; //4bytes
	 *
	 */

	bias_dram_read(bias_addr, dma_control, in_data, K);

	/*BIAS_DRAM_WRITE_K: for (unsigned int ko = 0; ko < K/VEC_SIZE; ko++) {
	#pragma HLS loop_tripcount min=1 max=1
	#pragma HLS pipeline off
		BIAS_DRAM_WRITE_H: for (unsigned int wh = 0; wh < WH*WH; wh++) {
		#pragma HLS loop_tripcount min=49 max=49
		#pragma HLS pipeline off*/

	unsigned int ko = 0;
	unsigned int wh = 0;
			bias_dram_write(output_addr,dma_control, out_data,
					ko, wh, WH, K);
	//	}
	//}
/*
...
/*
unsigned int input_addr = bias_addr;
//unsigned int output_addr = _addr;
unsigned int transfn = K;
unsigned int intervn = L1_TILENUM_C;
unsigned int n = C;
